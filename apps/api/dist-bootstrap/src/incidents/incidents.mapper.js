...
function collectSourceDocuments(row) {
    ...
    for (const { measurement } of row.measurements) {
        ...
        if (measurement.station.locationSourceDocument) {
            byId.set(measurement.station.locationSourceDocument.id, measurement.station.locationSourceDocument);
        }
    }
    ...
    for (const { candidateObject } of row.candidateObjects) {
        if (candidateObject.geometrySourceDocument) {
            byId.set(candidateObject.geometrySourceDocument.id, candidateObject.geometrySourceDocument);
        }
    ...
    if (row.upstreamStation?.locationSourceDocument) {
        byId.set(row.upstreamStation.locationSourceDocument.id, row.upstreamStation.locationSourceDocument);
    }
    if (row.downstreamStation?.locationSourceDocument) {
        byId.set(row.downstreamStation.locationSourceDocument.id, row.downstreamStation.locationSourceDocument);
    }
    return [...byId.values()]
        .sort((left, right) => compareText(left.id, right.id))
