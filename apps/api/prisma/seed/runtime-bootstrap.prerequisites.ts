...
export interface RuntimeBootstrapPrerequisiteSummary {
...
export async function verifyRuntimeBootstrapPrerequisites(
    transaction: Prisma.TransactionClient,
    plan: RuntimeBootstrapPlan,
): Promise<RuntimeBootstrapPrerequisiteSummary> {
    const sourceDocuments =
        await transaction.sourceDocument.findMany({
            where: {
                id: {
    ...
    assertExactIds(
        'station',
        stations,
        plan.requiredVerifiedData.stationIds,
    )

    const stationRelations =
        await transaction.stationRelation.findMany({
            where: {
                id: {
                    in: plan.requiredVerifiedData
                        .stationRelationIds,
    ...
    assertExactIds(
        'station relation evidence',
        relationEvidence,
        plan.requiredVerifiedData
            .stationRelationEvidenceIds,
    )

    verifyRelationEvidence(
        relationEvidence,
        plan.relations,
    )

    const measurements =
        await transaction.measurement.findMany({
            where: {
                id: {
                    in: plan.requiredVerifiedData
                        .measurementIds,
...
function verifySourceDocuments(
    ...
    for (const row of rows) {
        ...
        if (expected === undefined) {
            throw conflict(
                `Missing source plan for verified document ${row.id}`,
            )
        }
        ...
        assertEqual(
            row.originalUrl,
            expected.url,
            `source document ${row.id} original URL`,
        )

        assertEqual(
            row.canonicalUrl,
            expected.url,
            `source document ${row.id} canonical URL`,
        )

        assertEqual(
            row.publisher,
            expected.publisher,
            `source document ${row.id} publisher`,
        )

        assertEqual(
            row.sha256,
            expected.sha256,
            `source document ${row.id} SHA-256`,
        )

        assertEqual(
            normalizeContentType(row.mediaType),
            expected.contentType,
            `source document ${row.id} content type`,
        )
        ...
        if (
            ...
            throw conflict(
                `Verified source document ${row.id} is not marked official and verified in the handoff`,
            )
        }
    }
}

function verifyStationRelations(
    rows: readonly {
        id: string
        kind: string
...
function verifyRelationEvidence(
    ...
    const relationByEvidenceId =
        buildRelationByEvidenceId(
            relationPlans,
        )

    const foundSourcesByRelation =
        new Map<string, Set<string>>()

    for (const row of rows) {
        const expectedRelation =
            relationByEvidenceId.get(row.id)

        if (expectedRelation === undefined) {
            throw conflict(
                `Missing relation plan for evidence ${row.id}`,
            )
        }

        assertEqual(
            row.stationRelationId,
            expectedRelation.id,
            `relation evidence ${row.id} edge`,
        )
        ...
        if (
            ...
            throw conflict(
                `Relation evidence ${row.id} references an unexpected source document`,
            )
        }
        ...
        if (
            ...
            throw conflict(
                `Relation evidence ${row.id} has no verified source page`,
            )
        }
        ...
        const sourceIds =
            foundSourcesByRelation.get(
                expectedRelation.id,
            ) ?? new Set<string>()

        sourceIds.add(row.sourceDocumentId)

        foundSourcesByRelation.set(
            expectedRelation.id,
            sourceIds,
        )
    }
    ...
    for (const relation of relationPlans) {
        ...
        if (
            ...
            throw conflict(
                `Station relation ${relation.id} has conflicting evidence sources`,
            )
        }
    }
}
...
function verifyMeasurements(
    ...
    for (const row of rows) {
        assertEqual(
            row.verificationStatus,
            VerificationStatus.OFFICIAL,
            `measurement ${row.id} verification status`,
        )
    }
}

function buildRelationByEvidenceId(
    relationPlans:
        readonly RuntimeBootstrapRelationPlan[],
): Map<
    string,
...
> {
    ...
    for (const relation of relationPlans) {
        for (
            const evidenceId
            of relation.evidenceIds
        ) {
            if (result.has(evidenceId)) {
                throw conflict(
                    `Relation evidence ${evidenceId} belongs to multiple relation plans`,
                )
            }
...
function assertEqual(
    ...
    throw conflict(
        `Conflicting runtime bootstrap prerequisite: ${field}`,
    )
}
