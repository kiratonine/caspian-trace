...
export function findRelevantPages(pages: PdfPage[]): RelevantPage[] {
...
export function extractMeasurementCandidates(page: PdfPage): MeasurementCandidate[] {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const sourceExcerpt = nextLine !== undefined && /^(?:3|³)$/u.test(nextLine)
      ? `${line}\n${nextLine}`
      : line
    if (!INDICATOR_PATTERN.test(sourceExcerpt) || !UNIT_PATTERN.test(sourceExcerpt)) continue
...
export function validateCandidateAgainstText(
  ...
  if (!page.text.includes(candidate.sourceExcerpt)) return { valid: false, reason: 'EXCERPT_NOT_IN_PAGE' }
  if (!INDICATOR_PATTERN.test(candidate.sourceExcerpt)) return { valid: false, reason: 'INDICATOR_NOT_IN_EXCERPT' }
  if (!UNIT_PATTERN.test(candidate.sourceExcerpt)) return { valid: false, reason: 'UNIT_NOT_IN_EXCERPT' }
