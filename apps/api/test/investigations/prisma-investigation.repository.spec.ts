...
describe('PrismaInvestigationRepository runtime bootstrap input', () => {
  ...
  it('limits a shared candidate and source documents to the current incident', async () => {
    ...
    const may = await repository.loadInput('inv-atyrau-2025-05')
    expect(may?.candidateObjects[0]?.evidenceDocumentIds).toEqual([
      'doc-kazhydromet-2025-05',
    ])
    expect(may?.sourceDocuments.map(({ id }) => id)).toEqual([
      'doc-kazhydromet-2025-05',
    ])
    ...
    const september = await repository.loadInput('inv-atyrau-2025-09')
    expect(september?.candidateObjects[0]?.evidenceDocumentIds).toEqual([
  ...
  it.each([
  ...
  ])('reconstructs the manifest fixtureInputHash for %s', async (incidentId) => {
    ...
    expect(calculateInputHash(loaded!)).toBe(
      manifestCase?.expectedResult.fixtureInputHash,
    )
  })
  ...
  it('fails closed for malformed metadata and DB-to-metadata evidence conflicts', async () => {
    ...
    const mayIncident = malformed.incidentRows.find(
      ({ id }) => id === 'inv-atyrau-2025-05',
    )
    expect(mayIncident).toBeDefined()
    delete mayIncident!.metadata.runtimeBootstrap
    await expect(
      malformed.repository.loadInput('inv-atyrau-2025-05'),
    ).resolves.toBeNull()

    const conflicted = createHarness(inputs, handoff)
    const mayEvidence = conflicted.evidenceRows.find(
      ({ id }) => id === 'rel-may-asa-pair',
    )
    expect(mayEvidence).toBeDefined()
    mayEvidence!.sourceDocumentId = 'doc-kazhydromet-2025-09'
    await expect(
      conflicted.repository.loadInput('inv-atyrau-2025-05'),
    ).resolves.toBeNull()
    ...
    await expect(
      missingSource.repository.loadInput('inv-atyrau-2025-05'),
    ).resolves.toBeNull()
  })
})
...
function createHarness(
    ...
    input.measurements.map((measurement) => ({
      ...
      sourcePage:
        measurement.sourcePage === null
          ? null
          : { pageNumber: measurement.sourcePage },
  ...
  const incidentRows = inputs.map((input) => {
    const manifestCase = handoff.cases.find(
      ({ incident }) => incident.id === input.incident.id,
    )
  ...
  const fixtures = {
    loadInput: jest.fn(() => Promise.resolve(null)),
  }
  const repository = new PrismaInvestigationRepository(
    prisma as unknown as PrismaService,
    fixtures as unknown as FileInvestigationRepository,
  )
  return {
    repository,
    evidenceFindMany,
...
function scopedFindMany<T extends { id: string }>(
  ...
  return jest.fn(({ where }: FindManyArgs) =>
    Promise.resolve(filterByIds(rows, where.id.in)),
  )
}

function filterByIds<T extends { id: string }>(rows: T[], ids: string[]): T[] {
  const scope = new Set(ids)
  return rows.filter(({ id }) => scope.has(id)).sort((left, right) => left.id.localeCompare(right.id))
}
