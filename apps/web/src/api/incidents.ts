...
import type { IncidentDetail, IncidentListParams, IncidentSummary } from './contracts';
...
export async function fetchIncidents(params: IncidentListParams = {}): Promise<IncidentSummary[]> {
  warnStubOnce('GET /api/incidents — данные из ТЗ §5');
  const limit = Math.min(params.limit ?? MAX_INCIDENTS_LIMIT, MAX_INCIDENTS_LIMIT);
  return incidentSummaries
    .filter((incident) => (params.status ? incident.evidenceLevel === params.status : true))
    .filter((incident) => (params.region ? incident.region === params.region : true))
    .slice(0, limit);
}
