...
- Репозиторий: плоское Vite-приложение в корне (НЕ монорепа). Установлены React 19, TS strict,
  Tailwind 4, shadcn на Base UI (base-lyra), @fontsource Inter. НЕ установлены: TanStack Query,
  Zustand, react-router-dom — ставить в сессии 2.
- **Следующая задача (сессия 2): «Типы + константы + api-слой с заглушками»** — см. блок 2–24 ч.
- **Блокеры сессии 2:** ответы команды на вопросы 1 (порядок створов), 2 (incident vs
  investigation), 9 (монорепа). Без ответа на 2 слой заглушек не фиксировать; типы ТЗ §8
...
9. **Монорепа:** переезд в `apps/web` сейчас или отказ. ← решить до ~30-го часа, потом не трогаем
...
- [ ] **Типы + константы + api-слой с заглушками** ← СЛЕДУЮЩАЯ (сессия 2)
  ...
  - `.gitignore`: добавить `.env`, `.env.*`, разрешить `.env.example`;
