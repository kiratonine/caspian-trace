...
4. **Май/сентябрь:** как в данных связаны два расследования одного участка — признак для UI
   сравнения (главный «вау»-момент демо). ЧАСТИЧНО ЗАКРЫТ сессией 10 провизорно:
   в `IncidentSummary` добавлено поле `period` (ISO до месяца), сопоставимость выводится
   из `region + indicator + period !== null`. Команде подтвердить или дать настоящую
   связь (например, `siteId` участка) — правятся только `contracts.ts` и `seed-data.ts`.
...
- [ ] Печатное досье: маршрут `/dossier/:id`, `@media print`; print preview проверить сразу. ← СЛЕДУЮЩАЯ (сессия 11).
