...
**Сессия 9 (04.08.2026) — реплей: плеер, шкала, клавиатура. Завершена.**
...
- **Следующая задача (сессия 10): переключение май/сентябрь** с ЯВНОЙ
  визуальной сменой вывода (если выглядит как смена фильтра — провал, §5 и
  подводный камень 8 промпта). Клик по карточке уже переключает событие —
  нужен именно акцент на смене вывода (сравнение L2↔L3, было/стало).
...
- [ ] Переключение май/сентябрь с ЯВНОЙ визуальной сменой вывода (если выглядит как смена фильтра — провал). ← СЛЕДУЮЩАЯ (сессия 10).
...
- `.prettierrc` проекта (`semi: false`, `singleQuote: false`) расходится с кодовой базой,
  написанной в одинарных кавычках с `;`. `npm run format` переформатирует любой
  затронутый файл целиком — либо прогнать разово по всему `src` отдельным коммитом,
  либо привести конфиг к фактическому стилю. Решить до интеграции с бэком.
