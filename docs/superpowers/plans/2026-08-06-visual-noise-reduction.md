...
Ожидается: `[3, 0]` — «Атырауская область» исчезла с экрана полностью,
а «нефтепродукты» распадается на 2 + 1: два заголовка карточек (данные
с бэка, не наш текст) и подпись схемы, которая уйдёт в Task 3. Разбивку
по колонкам можно проверить прямо:

```js
const zone = (sel) =>
  document.querySelector(sel).innerText.split("нефтепродукты").length - 1
;[
  zone('section[aria-label="Сигналы и расследования"]'),
  zone('section[aria-label="Линейная схема реки"]'),
  zone('section[aria-label="Вывод и доказательства"]'),
]
```
...
const c = (re) => (table.innerText.match(re) || []).length
;[c(/нефтепродукты/g), c(/\bвода\b/g), c(/стр\. 22/g)]
```

Ожидается: `[0, 0, 0]` — в теле таблицы этих значений больше нет,
они стоят в подзаголовке над ней. До правки было `[7, 7, 7]`.

**Считать «воду» обязательно по границе слова.** Простой `split("вода")`
даёт ложные срабатывания: в названиях створов есть «осетрового заво́да»,
и подстрока «вода» находится внутри слова «завода». Проверять надо
колонку среды, а не любое вхождение букв.
