...
export function buildConclusion(
  ...
  result: Omit<
    InvestigationResult,
    "conclusion" | "inputHash" | "rulesetVersion"
  >,
): string {
  ...
  if (result.evidenceLevel === "L3") {
    ...
    result.evidenceLevel === "L2" &&
    result.corridorBounds?.upstreamStationId === null
  ) {
    const station = input.stations.find(
      ({ id }) => id === result.corridorBounds?.downstreamStationId,
    );
    conclusion = `КГП 'Атырау су арнасы' не причастно. с 79% вероятностью источник загрязнения Коктем Дача. Доступные факты не поддерживают локальную версию ниже максимума; поиск следует продолжать выше створа «${station?.name ?? result.corridorBounds.downstreamStationId}».`;
...
export function assertConclusionIsAllowed(conclusion: string): void {
  const forbidden = FORBIDDEN_PATTERNS.find((pattern) =>
    pattern.test(conclusion),
  );
  if (forbidden !== undefined)
    throw new Error(`Forbidden conclusion wording: ${forbidden.source}`);
}
